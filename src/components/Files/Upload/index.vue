        ...
        <div class="upload_warp_img_all">
          <div v-for="(item, key) in imgList" :key="key" class="upload_warp_img_div">
            <div class="upload_warp_img_div_top" @click="fileDel(key)">
              <svg-icon icon-class="close" class="upload_warp_img_div_del"/>
            </div>
            <img :src="item.file.src" :class="ISPHONE ? 'upload_warp_img_phone' : 'upload_warp_img_pc'" @click="enlargeClk(item)">
          </div>
          <div v-if="imgList.length < 6" class="upload_warp_left" @click="fileClick()">
            <svg-icon icon-class="addImage" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    console.log('是否是手机端=>', this.ISPHONE)
  },
  methods: {
    fileClick() {
      /* eslint-disable */
